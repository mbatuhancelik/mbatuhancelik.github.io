...
The binding constraint is the cost of collecting the data. Once an interaction has been sampled enough times the model learns it, so the informative portion of a randomly collected dataset stops growing long before the dataset does, and every further sample of an already-predicted transition still costs an execution. If an interaction occurs with probability $$p$$ under a uniform policy, obtaining the samples it needs takes on the order of $$1/p$$ executions, each a pick-and-place carried out in simulation or on hardware. Adding objects lowers $$p$$ for every composite interaction at once, so the budget required to cover the environment grows while the useful fraction of what is collected falls. The limit is therefore attributed to the exploration policy rather than to the capacity of the architecture: the interactions that would teach it are not reached within an affordable budget of simulation time, training time and energy.

Our previous work, [Developmental Scaffolding with Large Language Models](/publication/2023-icdl-scaffolding) (ICDL 2023), addresses the same bottleneck from outside the learner, using an LLM to choose among the actions the robot can execute. This project derives the guidance signal from the learner's own predictive uncertainty instead, which removes the dependence on a model that already knows something about the world.
...
A second theme is convoluted interactions which tests the relational module of the model. In the demonstration below, the agent constructs a bridge and then attempts to manipulate its base. Since the primitives are top-down, targeting a foundational block displaces the objects resting two layers above it, so predicting the transition requires resolving two layers of stacking alongside the proximity relations. The episode takes five consecutive actions to build the structure and further actions to manipulate it. Each action names an object to grasp and an object to place against, each with three proximity locations, so with $$n$$ objects, there are $$9n^2$$ available actions. Several sequences produce the same structure, so the figure is an estimate, but a chain of this length remains improbable under a uniform policy. However, it is observed during the last iteration of active exploration as previous steps exhausted trivial interactions.
...
<div style="text-align: center; margin: 20px auto; max-width:100%;">
  ...
  <p style="font-style: italic; color: #666; margin-top: 8px;">Fig. 5: Manipulation of a bridge through its foundation, requiring prediction over nested relations.</p>
</div>
